...
graph TB
    subgraph Planning["Planning (top-down — skills produce workspace files)"]
        ...
        A["L3: Architecture Skill<br/>→ &lt;MILESTONE&gt;_ARCH.md<br/>(working draft)"]
    ...
    subgraph Rollup["Architecture Roll-up (on milestone completion)"]
        RU["Merge working draft<br/>→ canonical arch docs<br/>(ARCH.md, *_ARCH.md)"]
    end
...
**Architecture docs have a two-phase lifecycle**: the architecture skill produces a **working draft** (`<MILESTONE>_ARCH.md`) during planning. Agents reference this draft during implementation. When the milestone completes, the working draft is **rolled up** into the canonical architecture docs — either promoted as a new subsystem doc, merged into the existing `ARCH.md`, or split into a cross-cutting concern doc. The canonical set always represents the current system state. Working drafts are archived to `docs/archive/` after roll-up.
